...
function renderChip(sym) {
  ...
  return `
    <div class="chip" data-id="${sym.id}">
      ...
      <div class="chip__label">${escapeHtml(label)}<br /><span style="color: var(--muted)">ID ${sym.id}</span></div>
...
function main() {
  const data = window.__TODDLER_DATA__;
  if (!data || !Array.isArray(data.symbols)) {
    throw new Error("Missing toddler data. Run scripts/build_toddler_translator_data.py");
  }
  ...
  sentence.addEventListener("click", (e) => {
    ...
    if (action === "remove") {
      selected = selected.filter((x, idx) => !(x === id && idx === selected.indexOf(id)));
      renderSentence();
      return;
    }
    if (action === "up") {
